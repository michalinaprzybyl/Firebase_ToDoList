...
// 6. Na form nałóż event listener (na submit).
// W środku EventListener'a:
// 7. Ściągnij wartości inputów email i password, zapisz do zmiennych (.value).
// 8. Wywołaj funkcję signInWithEmailAndPassword, funkcja przyjmuje 3 argumenty:
// - obiekt auth (zaimportuj z firebaseConfig.js)
// - email (który przed chwilą ściągnąłeś z inputa)
// - password (też z inputa)
// 9. Do wywołania poprzedniej funkcji dopisz then'a, będzie on przyjmował parametr creds, w środku then'a console.log(creds), console.log('Zalogowano') i wywołanie renderHomePage() (wcześniej trzeba zaimportować), dodaj metode catch i console.log('Error').
